...
function ChatContent() {
  ...
  }, [connect, loadConversations, openConversation, paramUserId, paramName]);
  ...
  const sendMessage = (e: React.FormEvent) => {
    ...
    setMessages(prev => [...prev, {
      ...
      message: newMessage.trim(),
      timestamp: new Date().toISOString(),
      order_id: activeOrder || undefined,
    }]);
    setNewMessage('');
  };
          ...
          {filtered.map(conv => (
            <div key={conv.user_id} className="flex items-center gap-3 px-4 py-3 cursor-pointer transition-colors border-b" style={{ borderColor: 'var(--border)', background: activePeer?.user_id === conv.user_id ? 'rgba(99,102,241,0.08)' : '', borderLeft: activePeer?.user_id === conv.user_id ? '2px solid var(--brand)' : '2px solid transparent' }} onClick={() => openConversation(conv)}>
              <div className="relative flex-shrink-0">
                <div className="w-10 h-10 rounded-full brand-gradient flex items-center justify-center text-white font-bold text-sm">{conv.username[0].toUpperCase()}</div>
                {conv.unread > 0 && <div className="absolute -top-1 -right-1 w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-bold text-white" style={{ background: 'var(--brand)' }}>{conv.unread}</div>}
              </div>
      ...
      <div className={`${!activePeer ? 'hidden md:flex' : 'flex'} flex-1 flex flex-col min-w-0 h-full`}>
        {activePeer ? (
          <>
            <div className="h-14 flex items-center px-4 md:px-5 gap-2 md:gap-3 border-b glass flex-shrink-0" style={{ borderColor: 'var(--border)' }}>
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <p className="text-[13px] font-semibold" style={{ color: 'var(--text-primary)' }}>{activePeer.username}</p>
                  {activeOrder && <span className="badge badge-pending text-[10px] px-1.5 py-0.5">Buyurtma #{activeOrder}</span>}
                </div>
            ...
            <form onSubmit={sendMessage} className="flex items-center gap-3 px-5 py-3 border-t glass" style={{ borderColor: 'var(--border)' }}>
              <input className="input flex-1 py-2.5" placeholder="Xabar yozing…" value={newMessage} onChange={e => setNewMessage(e.target.value)} />
              <motion.button type="submit" className="btn btn-primary w-10 h-10 p-0 rounded-xl flex-shrink-0" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} disabled={!newMessage.trim() || !connected} style={{ opacity: newMessage.trim() && connected ? 1 : 0.4 }}><Send size={16} /></motion.button>
            </form>
          </>
        ) : (
